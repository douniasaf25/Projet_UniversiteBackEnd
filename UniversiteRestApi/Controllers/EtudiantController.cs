...
namespace UniversiteRestApi.Controllers
{
    ...
    public class EtudiantController(IRepositoryFactory repositoryFactory) : ControllerBase
    {
        ...
        public async Task<ActionResult<EtudiantDto>> PutAsync(long id, [FromBody] EtudiantDto etudiantDto)
        {
            ...
            try
            {
                //  récupérer l'ancien étudiant en base
                var oldEtudiant = await new GetEtudiantByIdUseCase(repositoryFactory)
                    .ExecuteAsync(id);

                if (oldEtudiant == null) return NotFound();
                
                await updateUserUc.ExecuteAsync(oldEtudiant.Email, etudiantDto.Email);
